...
func main() {
	...
	config.AllowOrigins = []string{"localhost:3001"}


	models.ConnectDatabase() // new
	models.DB.AutoMigrate(&models.Manufacturer{})
	models.DB.AutoMigrate(&models.Machine{})
	models.DB.AutoMigrate(&models.Counter{})
	models.DB.AutoMigrate(&models.MachineType{})


	r.GET("/test",test)
	//r.GET("/API/counter/:machine_id/:mitarbeiter_id/:auftrags_id",counter) //Machine/Mitarbeiter/Auftrag
	r.GET("/API/counter/add/:machine_id/:mitarbeiter_id/:auftrags_id",counterAdd)
	r.GET("/API/counter/avg/:machine_id/:mitarbeiter_id/:auftrags_id/limit",getAvg)


	r.GET("/ping",TokenAuthMiddleware_(), pong)
	r.GET("/API/machines",controllers.GetMachines)
	r.GET("/API/Manufacture", controllers.GetManus)//Legacy
	r.GET("/API/Manufacturers", controllers.GetManus)
	r.GET("/loggeduser",TokenAuthMiddleware_(), getLoggedUser)
	r.GET("API/Types",controllers.GetTypes)

	r.POST("/answer",answer)
	r.POST("/login", Login)
	r.POST("/API/Manufacture",controllers.CreateManu)
	r.POST("API/Type",controllers.CreateType)
	r.POST("API/Machine",controllers.CreateMachine)

	r.DELETE("/API/Manufacture/:manuId",controllers.DeleteManu)
	r.DELETE("API/Type/:ID")
	r.Run() // listen and serve on 0.0.0.0:8080 (for windows "localhost:8080")
}


func getAvg(c *gin.Context){
	_machineId := (c.Param("machine_id"))
	_mitarbeiterId := c.Param("mitarbeiter_id")
	_auftragId := c.Param("auftrags_id")
	_limit :=c.Param("limit")

 var result float64
 models.DB.Raw("select get_avg_duration(?,?,?,?)",_machineId,_mitarbeiterId,_auftragId,_limit).Scan(&result)
 fmt.Println("AVG_Duration of:"+_limit+"is:"+fmt.Sprint(result))
}

func counterAdd(c *gin.Context)  {
	fmt.Println("<<<<counter>>>>")
	machineId := (c.Param("machine_id"))
	mitarbeiterId := c.Param("mitarbeiter_id")
	auftragId := c.Param("auftrags_id")

	var result int
	models.DB.Raw("select count_up_counter(?,?,?)",machineId,mitarbeiterId,auftragId).Scan(&result)
  fmt.Println("current Count("+machineId+"/"+mitarbeiterId+"/"+auftragId+")is:"+fmt.Sprint(result))
}
